    ...
    <div class="overflow-hidden rounded-xl border border-sidebar-border bg-card shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-sidebar-border">
                    <tr v-for="client in clients.data" :key="client.id" class="transition-colors hover:bg-muted/30">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div
                                    class="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg border border-sidebar-border bg-muted">
                                    <img v-if="client.media && client.media.length > 0" :src="client.media[0].url"
                                        :alt="client.name" class="h-full w-full object-cover" />
                                    <div v-else
        ...
        <div v-if="clients.last_page > 1"
            ...
            <div class="text-sm text-muted-foreground">
                Showing {{ (clients.current_page - 1) * clients.per_page + 1 }}
                to {{ Math.min(clients.current_page * clients.per_page, clients.total) }}
                of {{ clients.total }} results
            </div>
            <div class="flex items-center gap-2">
                <button @click="goToPage(clients.links[0]?.url)" :disabled="clients.current_page === 1"
                ...
                <div class="flex gap-1">
                    <button v-for="(link, index) in clients.links.slice(1, -1)" :key="index" @click="goToPage(link.url)"
                        :class="[
                ...
                <button @click="goToPage(clients.links[clients.links.length - 1]?.url)"
