    ...
    <div class="overflow-hidden rounded-xl border border-sidebar-border bg-card shadow-sm">
                ...
                <tbody class="divide-y divide-sidebar-border">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 flex-shrink-0 overflow-hidden rounded-lg border border-sidebar-border bg-muted">
                                    <img
                                        v-if="blog.media && blog.media.length > 0"
                                ...
                                <div class="flex flex-col">
                                    <span class="font-medium text-foreground">{{ blog.title }}</span>
                                    <span v-if="blog.details" class="mt-1 line-clamp-1 text-sm text-muted-foreground">
                                        {{ blog.details }}
                                    </span>
                                </div>
                            </div>
                    ...
                    <tr v-if="blogs.data.length === 0">
                        <td colspan="4" class="px-6 py-12 text-center">
                            <p class="text-muted-foreground">No blogs found</p>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        >
            <div class="text-sm text-muted-foreground">
                Showing {{ (blogs.current_page - 1) * blogs.per_page + 1 }}
                to {{ Math.min(blogs.current_page * blogs.per_page, blogs.total) }}
                of {{ blogs.total }} results
            </div>
            <div class="flex items-center gap-2">
                <button
                    @click="goToPage(blogs.links[0]?.url)"
                ...
                <div class="flex gap-1">
                    <button
                        v-for="(link, index) in blogs.links.slice(1, -1)"
                        :key="index"
                ...
                <button
                    @click="goToPage(blogs.links[blogs.links.length - 1]?.url)"
