...
const formData = ref({
    ...
    feature_count: props.data.feature_count || 4,
    ...props.data
});
...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div>
            ...
            <textarea v-model="formData.details" rows="4"
                class="w-full rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                placeholder="Explain your unique value proposition"></textarea>
        </div>

        <div>
            <label class="mb-1.5 block text-sm font-medium text-foreground">Number of Features</label>
            <input v-model.number="formData.feature_count" type="number" min="1" max="8"
                class="w-full rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
        </div>
    ...
    <div v-else class="space-y-2">
        <div class="rounded-lg bg-muted/30 p-4">
            <h3 class="mb-2 text-lg font-bold text-foreground">{{ formData.title || 'Why Us Section' }}</h3>
            <p v-if="formData.sub_title" class="mb-2 text-sm text-muted-foreground">{{ formData.sub_title }}</p>
            <p v-if="formData.details" class="text-sm text-foreground line-clamp-2">{{ formData.details }}</p>
            <div class="mt-2">
                <span
                    class="rounded-full bg-emerald-500/10 px-2 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
                    {{ formData.feature_count }} Features
                </span>
            </div>
        </div>
    </div>
