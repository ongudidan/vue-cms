...
const formData = ref({
    ...
    image_position: props.data.image_position || 'right',
    ...props.data
});
...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div>
            ...
            <input v-model="formData.image_url" type="text"
                class="w-full rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                placeholder="https://example.com/image.jpg" />
        </div>

        <div>
            <label class="mb-1.5 block text-sm font-medium text-foreground">Image Position</label>
            <select v-model="formData.image_position"
    ...
    <div v-else class="space-y-2">
        <div class="rounded-lg bg-muted/30 p-4">
            <h3 class="mb-2 text-lg font-bold text-foreground">{{ formData.title || 'Image Section' }}</h3>
            ...
            <div class="mt-2">
                <span
                    class="rounded-full bg-indigo-500/10 px-2 py-1 text-xs font-medium text-indigo-600 dark:text-indigo-400">
                    Image: {{ formData.image_position }}
                </span>
            </div>
        </div>
    </div>
