...
const formData = ref({
    ...
    show_all_testimonials: props.data.show_all_testimonials || true,
    testimonial_limit: props.data.testimonial_limit || 3,
    ...props.data
});
...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div class="flex items-center gap-3">
            <input id="show_all_testimonials" v-model="formData.show_all_testimonials" type="checkbox"
                class="h-4 w-4 rounded border-sidebar-border text-primary focus:ring-2 focus:ring-primary/20" />
            <label for="show_all_testimonials" class="text-sm font-medium text-foreground">Show All Testimonials</label>
        </div>

        <div v-if="!formData.show_all_testimonials">
            <label class="mb-1.5 block text-sm font-medium text-foreground">Number of Testimonials to Display</label>
            <input v-model.number="formData.testimonial_limit" type="number" min="1" max="20"
                class="w-full rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
        </div>
    ...
    <div v-else class="space-y-2">
        <div class="rounded-lg bg-muted/30 p-4">
            <h3 class="mb-2 text-lg font-bold text-foreground">{{ formData.title || 'Testimonials Section' }}</h3>
            ...
            <div class="mt-2">
                <span
                    ...
                    {{ formData.show_all_testimonials ? 'All Testimonials' : `${formData.testimonial_limit}
                    Testimonials` }}
                </span>
            </div>
        </div>
