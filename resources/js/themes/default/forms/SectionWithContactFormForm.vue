...
const formData = ref({
    ...
    show_map: props.data.show_map || false,
    ...props.data
});
...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div class="flex items-center gap-3">
            <input id="show_map" v-model="formData.show_map" type="checkbox"
                class="h-4 w-4 rounded border-sidebar-border text-primary focus:ring-2 focus:ring-primary/20" />
            <label for="show_map" class="text-sm font-medium text-foreground">Show Map</label>
        </div>
    ...
    <div v-else class="space-y-2">
        <div class="rounded-lg bg-muted/30 p-4">
            <h3 class="mb-2 text-lg font-bold text-foreground">{{ formData.title || 'Contact Form' }}</h3>
            ...
            <div v-if="formData.show_map" class="mt-2">
                <span
