...
const formData = ref({
    ...
    icon_style: props.data.icon_style || 'solid',
    ...props.data
});
...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div>
            <label class="mb-1.5 block text-sm font-medium text-foreground">Icon Style</label>
            <select v-model="formData.icon_style"
    ...
    <div v-else class="space-y-2">
        <div class="rounded-lg bg-muted/30 p-4">
            <h3 class="mb-2 text-lg font-bold text-foreground">{{ formData.title || 'Icon Box Section' }}</h3>
            ...
            <div class="mt-2">
                <span
                    class="rounded-full bg-orange-500/10 px-2 py-1 text-xs font-medium text-orange-600 dark:text-orange-400">
                    {{ formData.icon_style }} icons
                </span>
            </div>
        </div>
    </div>
