...
<template>
    <div v-if="editing" class="space-y-4">
        ...
        <div>
            <label class="mb-1.5 block text-sm font-medium text-foreground">Team Members</label>
            <div class="space-y-3">
                <div v-for="(member, index) in formData.members" :key="index"
                    ...
                    <div class="flex gap-2">
                        <input v-model="member.name" type="text"
                            ...
                            placeholder="Name" />
                        <input v-model="member.position" type="text"
                            class="flex-1 rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm"
                    ...
                    <input v-model="member.image" type="text"
                        class="w-full rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm"
                        placeholder="Image URL" />
                    <div class="grid grid-cols-3 gap-2">
                        <input v-model="member.twitter" type="text"
                            class="rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm"
                            placeholder="Twitter URL" />
                        <input v-model="member.linkedin" type="text"
                            class="rounded-lg border border-sidebar-border bg-background px-3 py-2 text-sm"
    ...
    <div v-else class="rounded-lg bg-muted/30 p-4">
        <h3 class="mb-2 text-lg font-bold">{{ formData.title || 'Team Grid' }}</h3>
        <p v-if="formData.subtitle" class="mb-2 text-sm text-muted-foreground">{{ formData.subtitle }}</p>
        <div class="text-xs text-muted-foreground">{{ formData.members.length }} team members</div>
    </div>
</template>
