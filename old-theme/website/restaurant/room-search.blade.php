...
@section('content')
   @include('website.shared.page-banner', ['title' => 'Room Search Results'])
      ...
      <div class="container-fluid w__80">
         ...
         <div class="row g-4">
            ...
            @if($rooms->count() > 0)
               ...
               @else
                  <div class="text-center py-5 w-100 bg-light">
            ...
            <div class="col-md-4 col-12">
               <div class="sidebar sticky-bar">
                  <div class="widget bg-light">
                     <div class="p-4 rounded-0 shadow-sm mb-4">
                           ...
                           <div class="mb-3 form-group">
                              ...
                              <select name="room_type_id" class="form-select rounded-0">
                                 ...
                                 @foreach($roomTypes as $type)
                                    <option value="{{$type->id }}" @if(request('room_type_id') === $type->id) selected @endif>{{$type->name}}</option>
                                 @endforeach
                              </select>
                           </div>
                           <div class="mb-3 form-group">
                              <label class="form-label">Capacity</label>
                              <input type="number" class="form-control rounded-0" name="capacity" value="{{ request('capacity') }}">
                           </div>
                           <div class="mb-3 form-group">
                              <label class="form-label">Price Per Day</label>
                              <input type="number" class="form-control rounded-0" name="price_per_day" value="{{ request('price_per_day') }}">
                           </div>
                           <div class="mb-3 form-group">
                              <label class="form-label">Price Per Hour</label>
                              <input type="number" class="form-control rounded-0" name="price_per_hour" value="{{ request('price_per_hour') }}">
                           </div>
                           <div class="mb-3 form-group">
                              ...
                              <select name="amenity_id" class="form-select rounded-0">
                                 ...
                                 @foreach($amenities as $amenity)
                                    <option value="{{$amenity->id }}" @if(request('amenity_id') === $amenity->id) selected @endif>{{$amenity->name}}</option>
                                 @endforeach
                              </select>
                           </div>
